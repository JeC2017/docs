...
本樣本的 BIC 選擇 VAR(`r p_selected`)，有效估計列數為 `r nrow(fit_jp$residuals)`，最大伴隨根為 `r sprintf("%.3f", max(Mod(eigen(companion_matrix(fit_jp$A))$values)))`。根在單位圓內只表示這個估計規格的動態穩定；它不保證殘差已成為白噪音。下一步應先看模型還漏掉哪些時間相依，再決定能否賦予曲線較強的結構解讀。
...
診斷確實留下警訊：`YieldChange` 方程的殘差 Ljung–Box p 值為 `r sprintf("%.4f", resid_diag$p_residual[resid_diag$equation == "YieldChange"])`，表示仍有線性相依值得追查；`OilChange` 方程的平方殘差 p 值為 `r formatC(resid_diag$p_squared[resid_diag$equation == "OilChange"], format = "e", digits = 2)`，則強烈提醒油價變動的波動不是固定不變。因此 VAR(1) 只能當作 BIC 基準。若要正式做區間推論，應比較較長落後階數、檢查異常月份，並採能容許條件異質變異的方法。
