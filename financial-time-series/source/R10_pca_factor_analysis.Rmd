...
評分沿用估計期的中心與尺度，在標準化空間比較實現值 \(Z\) 與重建值 \(\widehat Z\)。這樣估計期的 80% 選維規則與較晚月份的重建比例使用同一尺度；原始百分點 MSE 另列一欄，不能和解釋變異比例混為一談。三成分在驗證期重建約 `r sprintf("%.1f", 100 * validation_score["standardized_fraction_reconstructed"])`% 的標準化變動，顯示估計期找到的低維空間並未在下一段月份立即失效。這是同月重建品質，不是報酬方向的預測準確率。
...
測試期原始百分點 MSE 約為 `r sprintf("%.2f", test_score["original_scale_MSE"])`，在與選維規則一致的標準化尺度上，重建比例約為 `r sprintf("%.1f", 100 * test_score["standardized_fraction_reconstructed"])`%。請再次注意，程式先看到目標月份五檔股票的同月報酬，才計算該月主成分分數與重建；它回答「少數共同方向能保留多少同月資訊」，不是以前一期資訊預測下一期報酬。
